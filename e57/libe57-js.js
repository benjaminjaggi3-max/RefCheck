...
function findWasmBinary() {
  if (Module["locateFile"]) {
    return locateFile("libe57-js.wasm");
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL("libe57-js.wasm", import.meta.url).href;
}
...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("libe57-js.js", import.meta.url), {
      "type": "module",
      // This is the way that we signal to the node worker that it is hosting
      // a pthread.
      "workerData": "em-pthread",
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, 1, dirfd, path, buf, bufsize);
  try {
    path = SYSCALLS.getStr(path);
    path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, fd, message, length, flags, addr, addr_len);
  try {
    var sock = getSocketFromFD(fd);
    if (!addr) {
...
function ___syscall_socket(domain, type, protocol) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, domain, type, protocol);
  try {
...
function _getaddrinfo(node, service, hint, out) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, node, service, hint, out);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_connect, ___syscall_dup, ___syscall_fcntl64, ___syscall_getcwd, ___syscall_ioctl, ___syscall_openat, ___syscall_readlinkat, ___syscall_recvfrom, ___syscall_rmdir, ___syscall_sendto, ___syscall_socket, ___syscall_unlinkat, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write, _getaddrinfo ];
...
function _emjs_call_resolve(resolve_fn, value) {
  Emval.toValue(resolve_fn)(Emval.toValue(value));
}

function _emjs_call_reject(reject_fn, msg) {
  Emval.toValue(reject_fn)(new Error(UTF8ToString(msg)));
}

function _emjs_array_view(ptr, size, elementSize) {
  var arr = (growMemViews(), HEAPU8).subarray(ptr, ptr + size * elementSize);
  var freed = false;
  arr.free = function() {
    if (!freed) {
      freed = true;
  ...
  if (!globalThis._e57Finalizer) globalThis._e57Finalizer = new FinalizationRegistry(function(freeWasmMemory) {
    freeWasmMemory();
  });
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ _emjs_array_view,
    /** @export */ _emjs_call_reject,
    /** @export */ _emjs_call_resolve,
    /** @export */ _emjs_make_deferred,
    ...
    /** @export */ _emval_invoke_i64: __emval_invoke_i64,
    ...
    /** @export */ invoke_iij,
    /** @export */ invoke_iiji,
    /** @export */ invoke_iijiiiiiii,
    /** @export */ invoke_ji,
    /** @export */ invoke_jiii,
    /** @export */ invoke_jiiiijijii,
    /** @export */ invoke_v,
    /** @export */ invoke_vi,
    /** @export */ invoke_vii,
    /** @export */ invoke_viii,
...
function invoke_jiiiijijii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9) {
  var sp = stackSave();
  try {
    return getWasmTableEntry(index)(a1, a2, a3, a4, a5, a6, a7, a8, a9);
  } catch (e) {
...
function invoke_viiiiijijii(index, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10) {
  var sp = stackSave();
  try {
